...
def get_mapping_by_device(device: TuyaBLEDevice) -> list[TuyaBLEButtonMapping]:
    ...
    if not category:
        _LOGGER.debug(
            "No button mapping found for device category: %s",
            device.category,
        )
        return []
    ...
    if category.products:
        ...
        if product_mapping:
            ...
            _LOGGER.debug(
                "No button product mapping found for product_id: %s (category: %s). Falling back to category mapping.",
                device.product_id,
                device.category,
            )
    ...
    _LOGGER.debug(
        "No default button mapping available for category: %s (product_id: %s)",
        device.category,
        device.product_id,
    )
    return []
