...
from ._ble import TuyaBLEDataPoint, TuyaBLEDataPointType, TuyaBLEDevice
...
def get_mapping_by_device(device: TuyaBLEDevice) -> list[TuyaBLEClimateMapping]:
    ...
    if not category:
        _LOGGER.debug("No climate mapping for category: %s", device.category)
        return []
    ...
    if category.products:
        ...
        if product_mapping:
            ...
            _LOGGER.debug(
                "No product-specific climate mapping for product_id: %s in category: %s. Falling back to generic.",
                device.product_id,
                device.category,
            )
    ...
    _LOGGER.debug(
        "No default climate mapping for category: %s (product_id: %s)",
        device.category,
        device.product_id,
    )
    return []
...
class TuyaBLEClimate(TuyaBLEEntity, ClimateEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug("Error updating HVAC action: %s", e)
