...
MAX_PACKET_LENGTH = 4096  # safeguard limit for assembled BLE packets
...
class TuyaBLEDevice:
    ...
    def _build_pairing_request(self) -> bytes:
        if not self._device_info or not self._local_key:
...
async def _update_device_info(self) -> bool:
    ...
    def get_or_create_datapoint(
        ...
        """Return an existing datapoint or create a new one if it doesn't exist."""
    ...
    def register_connected_callback(
        ...
        """Register a callback to be called when device connected."""
    ...
    async def _execute_disconnect(self) -> None:
        ...
        async with self._connect_lock:
            ...
            if client and client.is_connected:
                try:
                    await asyncio.wait_for(client.stop_notify(CHARACTERISTIC_NOTIFY), timeout=5)
                except asyncio.TimeoutError:
                    _LOGGER.warning("%s: stop_notify timed out", self.address)

                try:
                    await asyncio.wait_for(client.disconnect(), timeout=5)
                except asyncio.TimeoutError:
                    _LOGGER.warning("%s: disconnect timed out", self.address)
        async with self._seq_num_lock:
            self._current_seq_num = 1

    async def _try_with_backoff(
        self,
        attempt_fn: Callable[[], Awaitable[bool]],
        max_attempts: int = 5,
    ) -> bool:
        for i in range(max_attempts):
            try:
                ...
                _LOGGER.debug(
                    "%s: Attempt %d failed, backing off", self.address, i + 1, exc_info=True
                )
...
async def _ensure_connected(self) -> None:
    ...
    async with self._connect_lock:
            ...
            try:
                async with global_connect_lock:
                    ...
                    try:
                        ...
                        self._client = None
                        _LOGGER.error("%s: starting notifications failed", self.address, exc_info=True)
                        return False

                    _LOGGER.debug("%s: Sending device info request", self.address)
                    if not await self._send_packet_while_connected(
                        TuyaBLECode.FUN_SENDER_DEVICE_INFO, bytes(0), 0, True
                    ):
                        self._client = None
                        _LOGGER.error("%s: Sending device info request failed", self.address)
                        return False
                    ...
                    if not await self._send_packet_while_connected(
                        ...
                        self._client = None
                        _LOGGER.error("%s: Sending pairing request failed", self.address)
                        return False
                ...
                self._client = None
                _LOGGER.debug("%s: BLE attempt failed, backing off", self.address, exc_info=True)
                return False
            except Exception:
                self._client = None
                _LOGGER.error("%s: Unexpected error during BLE connect", self.address, exc_info=True)
                return False

        if not await self._try_with_backoff(_attempt_connection, max_attempts=5):
            _LOGGER.error("%s: Connection failed after retries; RSSI: %s", self.address, self.rssi)
        ...
        if self._client and self._client.is_connected:
            ...
            _LOGGER.error("%s: Failed to establish connection", self.address)
    ...
    async def _reconnect(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug(
                "%s: Reconnect failed, backing off", self.address, exc_info=True
            )
            await asyncio.sleep(BLEAK_BACKOFF_TIME)
    ...
    @staticmethod
    def _pack_varint(value: int) -> bytearray:
        curr_byte: int
        result = bytearray()
        while True:
    ...
    @staticmethod
    def _unpack_varint(data: bytes, start_pos: int) -> tuple[int, int]:
        result: int = 0
        offset: int = 0
        while offset < 5:
    ...
    def _build_packets(
        ...
        use_login = code.uses_login_key()
        key = self._login_key if use_login else self._session_key
        security_flag = b"\x04" if use_login else b"\x05"

        if key is None:
            raise TuyaBLEDeviceError(0xFFFF)
        ...
        while pos < length:
            packet = bytearray()
            packet += self._pack_varint(packet_num)

            if packet_num == 0:
                packet += self._pack_varint(length)
    ...
    def _get_key(self, security_flag: int) -> bytes:
        ...
        if security_flag == 5:
            return self._session_key or hashlib.md5(self._local_key or b"").digest()
        return b"\x00" * 16  # fallback for unknown security_flag
    ...
    def _parse_datapoints_v3(
        ...
        while len(data) - pos >= 4:
            ...
            _LOGGER.info("[DPS] %s #%s (%s): %s", self.address, id, type.name, value)
    ...
    def _parse_input(self) -> None:
        ...
        if _LOGGER.isEnabledFor(logging.DEBUG):
            _LOGGER.debug("%s: Decrypted payload: %s", self.address, raw.hex())
    ...
    def _notification_handler(self, _sender: int, data: bytearray) -> None:
        ...
        packet_num, pos = self._unpack_varint(data, pos)
        ...
        if packet_num == self._input_expected_packet_num:
            if packet_num == 0:
                ...
                self._input_expected_length, pos = self._unpack_varint(data, pos)
                pos += 1
        ...
        if self._input_expected_length > 4096:
            _LOGGER.warning(
                "%s: Incoming packet too large (%s bytes). Dropping.",
                self.address,
                self._input_expected_length,
            )
            self._clean_input()
    ...
    async def _send_datapoints_v3(self, datapoint_ids: list[int]) -> None:
        ...
        for dp_id in datapoint_ids:
            ...
            if len(value) > 255:
                raise TuyaBLEDataLengthError()
            data.extend(pack(">BBB", dp.id, int(dp.type.value), len(value)))
            data.extend(value)
