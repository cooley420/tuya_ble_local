...
def get_mapping_by_device(device: TuyaBLEDevice) -> list[TuyaBLEBinarySensorMapping]:
    ...
    if not category:
        _LOGGER.debug(
            "No binary_sensor category mapping for device category: %s",
            device.category,
        )
        return []
    ...
    if category.products:
        ...
        if product_mapping:
            ...
            _LOGGER.debug(
                "No product-specific mapping for product_id: %s (category: %s). "
                "Falling back to category default mapping.",
                device.product_id,
                device.category,
            )
    ...
    _LOGGER.debug(
        "Category '%s' has no mapping list or product match for product_id '%s'",
        device.category,
        device.product_id,
    )
    return []
